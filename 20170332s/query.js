...
module.exports={
    sequelize:sequelize,
    search:async function (table_name, field, key) {
        if(arguments.length==3) {
            ...
            let data = await sequelize.model(table_name).findAll({obj2});
            //console.log(data);
            return data;
        }else{
            let data = await sequelize.model(table_name).findAll();
            return data;
        }
        /*if(arguments.length==3){
            let str;
            if(typeof key !=="number"){
                str = "SELECT * FROM "+table_name+" WHERE "+field+" = '"+key+"';" ;
            }else{
                str = "SELECT * FROM "+table_name+" WHERE "+field+" = "+key+";" ;
            }

            let data = await sequelize.query(str);
            //console.log(data);
            return data;
        }else{
            let data = await sequelize.model(table_name).findAll();
    ...
    insert:async function (table_name,field,value) {
        let table = await sequelize.model(table_name);
        let obj={};
        obj[field]=value.object.replace(/[ \b\f\n\r\t]/g, '');
        table.create(obj);
    },
    delete:async function (table_name,field,key){
        let obj1={};
        let obj2={};
        obj1[field]=key;
        obj2["where"]=obj1;
        await sequelize.model(table_name).destroy(obj2);
        /*sequelize.model(table_name).destroy({
            'where': {'id': [2, 3, 4]}
        });*/
    }
};